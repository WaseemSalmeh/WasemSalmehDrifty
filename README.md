...
I started by setting up the project and learning how I wanted to use Unity and GitHub together while building the game. At first it was just an empty racing idea, but I wanted it to turn into something playable instead of only a simple test scene. I made sure the project was saved properly, kept the files inside the Unity project organized, and used GitHub commits to keep track of the progress as the game changed from a blank project into a small drifting racing game.
...
Once the main gameplay worked, I added more vehicles and created the main menu. The menu includes Play, Choose Vehicle, and Choose Map. I wanted the menu to feel like a racing game, so I added bold colors, racing stripes, buttons, vehicle previews, and best time text. At one point the buttons could not be clicked, so I fixed the UI interaction setup to make the menu usable.
