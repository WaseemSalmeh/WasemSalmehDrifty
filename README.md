...
Drifty is a third-person racing game where the player drives a car around different tracks and tries to finish the lap as fast as possible. The player can choose between vehicles and maps, drift around corners, pass checkpoints, and try to beat their best time.

Core Loop

Player chooses a car and map -> race starts after the countdown -> player drives through checkpoints -> timer keeps counting -> player finishes the lap -> best time updates -> player restarts or chooses another car/map -> repeat.

Win & Lose Conditions

The player wins by driving through all checkpoints and crossing the finish line to complete the lap. There is no traditional lose screen, but the player can lose time by going off the track, missing checkpoints, drifting badly, or crashing, and they can press R to restart and try for a better lap.
